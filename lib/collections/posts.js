...
Meteor.methods({
  postInsert: function(postAttributes) {
    console.log('hello from postInsert!');
  ...
  upvote: function(postId) {
    // check inputs correct types
    check(this.userId, String);
    check(postId, String);
    var post = Posts.findOne(postId);
    // Must upvote an existing post
    if (!post)
      throw new Meteor.Error('invalid', 'Post not found!');
    // User shouldn't be able to upvote same post twice
    if (_.include(post.upvoters, this.userId))
      throw new Meteor.Error('invalid', 'Already updated this post!');
  ...
  postUpdate: function(postAttributes, postId) {
    ...
    return Posts.update(postId, {$set: postAttributes});
  }
});
